UNIT Queue;
INTERFACE
  PROCEDURE EmptyQ;{открывает очередь}
  PROCEDURE AddQ (VAR Elt : CHAR);{Добавляет элемент в очередь}
  PROCEDURE DelQ;{Удаляет первый элемент в очереди}
  PROCEDURE HeadQ(VAR Elt: CHAR);{Возвращает первый элемент очереди}
  PROCEDURE WriteQ;{Выводит всю очередь}
